...
// components
import { dndGridContainer, dndGridBox, dndGridUtils } from 'modules/dnd-grid';
...
export default {
	...
	data() {
		return {
			...
			elementSettingsModel: this.$root.elementSettingsModel,
			items: [],
			mainEl: null,
			loaded: false,
			backingGrid: false
	...
	methods: {
		...
		-------------------------------*/
		addElementModelListeners() {
			this.elementSettingsModel.on('change', data => {
				if (data.changed.hasOwnProperty('colNum')) {
		...
		setLayoutData() {
			for (let layoutKey in this.layoutData) {
				this.updateOption('layout-data' + this.getBreakpointPostfix(layoutKey), JSON.stringify(this.layoutData[layoutKey]));
			};
		},
