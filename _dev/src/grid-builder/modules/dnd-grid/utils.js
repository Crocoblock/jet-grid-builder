...
export const positionToPixels = (position, cellSize, margin = 0, outerMargin = 0) => {
	return {
		x: Math.round((position.x * cellSize.w) + ((position.x) * margin) + outerMargin),
		y: Math.round((position.y * cellSize.h) + ((position.y) * margin) + outerMargin),
		w: Math.round((position.w * cellSize.w) + ((position.w - 1) * margin)),
		h: Math.round((position.h * cellSize.h) + ((position.h - 1) * margin))
	};
};
